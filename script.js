  ...
  function openExpansion() {
    ...
    popup.innerHTML = `
      ...
      <button class="close-btn" onclick="closeExpansion()">&times;</button>
  ...
  function openLaws() {
    ...
    backdrop.className = 'modal-backdrop';
    backdrop.style.display = 'block';
    backdrop.style.zIndex = '2002';
    const popup = document.createElement('div');
    popup.className = 'menu-popup';
    popup.id = 'laws-popup';
    popup.style.display = 'block';
    popup.style.zIndex = '2003';
    popup.innerHTML = `
      <h2>National Laws & Policies</h2>
      <button class="close-btn" onclick="closeLaws()">&times;</button>
      <div class="menu-tabs">
  ...
  function loadLawsContent(tab) {
    ...
    const laws = {
      domestic: [{
        ...
        description: 'Establish comprehensive healthcare system accessible to all citizens',
        effect: '+10 Stability, -5 GDP'
      }, {
        name: 'Mandatory Military Service',
        description: 'Require all eligible citizens to serve in the armed forces',
        effect: '+15 Military Power, -5 Stability'
      }, {
        name: 'Free Education',
        description: 'Provide free public education from primary through university level',
        effect: '+5 Technology, -3 GDP'
      }],
      foreign: [{
        name: 'Open Borders',
        description: 'Remove restrictions on movement of people and goods across borders',
        effect: '+10 Relations, +5 GDP'
      }, {
        name: 'Trade Restrictions',
        description: 'Implement protective tariffs and trade barriers',
        effect: '-5 Relations, +10 GDP'
      }, {
        name: 'Military Alliance',
        description: 'Form strategic military partnerships with allied nations',
        effect: '+20 Military Power, -10 GDP'
      }],
      economic: [{
        name: 'Free Market',
        description: 'Reduce government intervention in economy to boost growth',
        effect: '+15 GDP, -5 Stability'
      }, {
        name: 'State Control',
        description: 'Establish government oversight of key economic sectors',
        effect: '+10 Stability, -10 GDP'
      }, {
        name: 'Mixed Economy',
        description: 'Balance free market principles with strategic regulation',
    ...
    content.innerHTML = laws[tab].map(law => `
      <div class="law-item">
        <h4>${law.name}</h4>
        <p>${law.description}</p>
        <p><strong>Effects:</strong> ${law.effect}</p>
  ...
  function switchLawTab(tab) {
    const tabs = document.querySelectorAll('#laws-popup .menu-tab');
    tabs.forEach(t => {
      t.classList.remove('active');
      t.style.background = '#ffffff';
      t.style.color = '#333333';
    });
    const activeTab = Array.from(tabs).find(t => t.textContent.toLowerCase().includes(tab));
    if (activeTab) {
      activeTab.classList.add('active');
      activeTab.style.background = '#0066ff';
  ...
  function enactLaw(lawName) {
    ...
    alert.textContent = `Law "${lawName}" has been enacted!`;
  ...
  function updateRank() {
    try {
      ...
      if (rankBarElement) {
        ...
        rankBarElement.style.width = `${progressToNext}%`;
      }
    } catch (error) {
  ...
  function createCountry() {
    ...
    if (newsFeed) {
      ...
      newsFeed.style.height = 'calc(100% - 180px)';
    }
    ...
    document.getElementById('government-type').style.display = 'none';
    document.getElementById('government-type-change').style.display = 'block';
    document.getElementById('government-type-change').addEventListener('change', changeGovernment);
    worldTension = 0;
    simulateAICountries();
    updateWorldTension();
    saveGameState();
  ...
  function updateStats(stats) {
    try {
      ...
      if (elements.researchPoints) {
        const techLevel = getCurrentTechLevel();
        const researchPoints = Math.round(countryTraits.technology * (techLevel.level + 1) * 10);
    ...
    try {
      ...
      if (stability && stability.nextElementSibling) {
        ...
        if (trendIndicator) {
          ...
          trendIndicator.innerHTML = `${researchPoints}%`;
        }
        validateStability();
      }
      if (influence) {
        ...
        const dipCount = diplomaticCount ? diplomaticCount.textContent : '0';
        const influenceLevel = getInfluenceLevel(countryTraits.influence);
        const influenceDetailsDiv = influence.nextElementSibling.getElementsByClassName('tech-details')[0];
        if (influenceDetailsDiv) {
          influenceDetailsDiv.innerHTML = `
            <div class="influence-level-info">
              <span class="influence-level">${influenceLevel.name}</span>
              <span class="influence-desc">${influenceLevel.description}</span>
              <div class="influence-benefits">
                ${influenceLevel.benefits.map(b => `<span class="benefit">${b}</span>`).join('')}
              </div>
            </div>
          `;
        }
        const influenceIndicator = influence.nextElementSibling.getElementsByClassName('trend-indicator')[0];
        if (influenceIndicator) {
          influenceIndicator.className = 'trend-indicator';
          influenceIndicator.innerHTML = `<span style="color: #0066ff;">${dipCount} Relations</span>`;
        }
      }
      if (technology) {
        technology.textContent = `${countryTraits.technology}%`;
        const techLevel = getCurrentTechLevel();
        const nextLevel = Object.values(technologyLevels).find(level => level.threshold > countryTraits.technology);
        const techProgress = nextLevel ? ((countryTraits.technology - techLevel.threshold) / (nextLevel.threshold - techLevel.threshold) * 100).toFixed(1) : 100;
        const techIndicator = technology.nextElementSibling.getElementsByClassName('trend-indicator')[0];
        if (techIndicator) {
          techIndicator.className = 'trend-indicator trend-up';
          techIndicator.innerHTML = `${techLevel.name} (${techProgress}%)`;
        }
      }
      if (stabilityProgress) stabilityProgress.style.width = `${countryTraits.stability}%`;
      if (influenceProgress) influenceProgress.style.width = `${countryTraits.influence}%`;
      if (technologyProgress) technologyProgress.style.width = `${countryTraits.technology}%`;
      const techLevel = getCurrentTechLevel();
      if (technology && technology.nextElementSibling) {
        const techDetailsDiv = technology.nextElementSibling.getElementsByClassName('tech-details')[0];
        if (techDetailsDiv) {
          techDetailsDiv.innerHTML = `
            <div class="tech-level-info">
              <span class="level-name">${techLevel.name}</span>
              <span class="level-desc">${techLevel.description}</span>
              <div class="tech-benefits">
                ${techLevel.benefits.map(b => `<span class="benefit">${b}</span>`).join('')}
              </div>
            </div>
          `;
        }
  ...
  function addNewsItem(text, type = 'normal', fullArticle = null) {
    ...
    newsItem.innerHTML = `
      <div class="news-header" style="margin-bottom: 8px;">
        ${importance}
        <div class="timestamp" style="color: #666; font-size: 0.85em;">${timestamp}</div>
      </div>
      <h4 style="color: ${type === 'critical' ? '#ff4444' : '#0066ff'}; margin-bottom: 10px;">${text}</h4>
      <div class="preview" style="color: #333; line-height: 1.5;">${(fullArticle || text).substring(0, 100)}${(fullArticle || text).length > 100 ? '...' : ''}</div>
    `;
  ...
  function declareWarBetween(country1, country2) {
    ...
    if (countryTraits[country1]) {
      countryTraits[country1].stability = Math.max(0, countryTraits[country1].stability - (10 + 5 * tensionMultiplier));
    }
    if (countryTraits[country2]) {
      countryTraits[country2].stability = Math.max(0, countryTraits[country2].stability - (10 + 5 * tensionMultiplier));
    }
  ...
  function updateWorldTension() {
    ...
    if (worldTension >= 75) {
      countryTraits.stability = Math.max(0, countryTraits.stability - 3);
      const currentGdp = parseFloat(document.getElementById('gdp').textContent.replace('$', '').replace('B', ''));
      ...
      countryTraits.stability = Math.max(0, countryTraits.stability - 1.5);
      const currentGdp = parseFloat(document.getElementById('gdp').textContent.replace('$', '').replace('B', ''));
      document.getElementById('gdp').textContent = `$${Math.max(0, currentGdp - 0.2).toFixed(1)}B`;
    } else if (worldTension <= 25) {
      countryTraits.stability = Math.min(100, countryTraits.stability + 1);
      const currentGdp = parseFloat(document.getElementById('gdp').textContent.replace('$', '').replace('B', ''));
      ...
      <div class="rank-benefits">
        ...
        <ul>
          ${currentRank.benefits.map(benefit => `
            <li>
              <div style="font-weight: 500;">${benefit}</div>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const countryHeader = document.querySelector('.country-header');
    if (countryHeader) {
      countryHeader.style.display = 'none';
    }
    if (instructionPopup) {
      instructionPopup.remove();
      instructionPopup = null;
  ...
  function showConfirmationPopup(title, message) {
    ...
    popup.innerHTML = `
      <h3 style="font-size: 1.2em; margin-bottom: 8px;">${title}</h3>
      <p style="font-size: 0.95em;">${message}</p>
    `;
    document.body.appendChild(popup);
  ...
  function dismissTip() {
    if (activeTip) {
      activeTip.style.transition = 'opacity 0.3s ease-out, transform 0.3s ease-out';
      activeTip.style.opacity = '0';
      activeTip.style.transform = 'translateY(20px)';
      setTimeout(() => {
        activeTip.remove();
        activeTip = null;
      }, 300);
  ...
  const countryRanks = {
    SETTLEMENT: {
      ...
      name: "Settlement",
      icon: "",
      benefits: ["Territory control", "Basic governance"]
    },
    TRIBE: {
      threshold: 20,
      name: "Tribe",
      icon: "",
      benefits: ["Cultural growth", "Resources"]
    },
    KINGDOM: {
      threshold: 40,
      name: "Kingdom",
      icon: "",
      benefits: ["Authority", "Taxation", "Military"]
    },
    EMPIRE: {
      threshold: 60,
      name: "Empire",
      icon: "",
      benefits: ["Expansion", "Army", "Trade"]
    },
    EMPIRE_STATE: {
      threshold: 80,
      name: "Empire State",
      icon: "",
      benefits: ["Global power", "Tech", "Economics"]
    },
    SUPREME_COUNTRY: {
      threshold: 90,
      name: "Supreme Country",
      icon: "",
      benefits: ["Superpower", "Nuclear", "Leadership"]
    },
    WORLD_POWER: {
      threshold: 100,
      name: "World Power",
      icon: "",
      benefits: ["Ultimate power", "Military might", "Economic rule"]
    }
  };
  function checkEmpireRestoration() {
    ...
    Object.keys(possibleEmpires).forEach(country => {
      ...
      if (possibleEmpires[country].required.every(req => relationships[country][req] && relationships[country][req] > 75)) {
        ...
        countryTraits[country].stability = Math.min(100, countryTraits[country].stability + 20);
  ...
  function changeGovernment(event) {
    const newGovt = event.target.value;
    const countryName = document.getElementById('country-name').value;
    const oldGovt = document.getElementById('government-type').value;
    countryTraits.stability = Math.max(0, countryTraits.stability - 50);
    Object.keys(relationships[countryName]).forEach(country => {
      if (ideologies[country]) {
        if (ideologies[country] === 'democratic' && newGovt === 'democracy' || ideologies[country] === 'communist' && newGovt === 'communist' || ideologies[country] === 'authoritarian' && ['dictatorship', 'monarchy', 'oligarchy'].includes(newGovt)) {
          relationships[countryName][country] += 20;
        } else {
          relationships[countryName][country] -= 20;
        }
      }
    ...
    worldTension += 15;
    const alert = document.createElement('div');
    alert.className = 'government-alert';
    alert.textContent = `Government changed to ${newGovt}`;
    document.body.appendChild(alert);
    alert.style.display = 'block';
    addNewsItem(`${countryName} has changed its government to ${newGovt}`, 'critical', `In a dramatic shift of power, ${countryName} has transformed its government from ${oldGovt} to ${newGovt}. ` + `This change has caused significant internal instability and drawn varied reactions from the international community.`);
    document.getElementById('government-type').value = newGovt;
    updateTraits();
    updateRank();
    updateWorldTension();
    saveGameState();
  ...
  const additionalIdeologies = {
    ...
    "Greenland": "democratic"
  };
  Object.assign(ideologies, additionalIdeologies);
  const technologyLevels = {
    PRIMITIVE: {
      level: 0,
      name: "Primitive",
      description: "Basic tools",
      benefits: ["Agriculture", "Tools"],
      threshold: 0
    },
    MEDIEVAL: {
      level: 1,
    ...
    MODERN: {
      ...
      benefits: ["Digital", "Manufacturing"],
      threshold: 60
    },
    INFORMATION: {
      level: 4,
    ...
    ADVANCED: {
      ...
      benefits: ["AI", "Space tech"],
      threshold: 90
    }
  };
  function getCurrentTechLevel() {
    const techValue = countryTraits.technology;
    let currentLevel = technologyLevels.PRIMITIVE;
    for (const [key, level] of Object.entries(technologyLevels)) {
      if (techValue >= level.threshold) {
        currentLevel = level;
      } else {
        break;
      }
  ...
  function startGame() {
    ...
    if (gameHeader) {
      ...
      gameHeader.style.animation = 'fadeIn 0.5s ease-out';
    }
    const themeToggle = document.querySelector('.theme-toggle');
    if (themeToggle) {
      themeToggle.style.display = 'flex';
      themeToggle.style.animation = 'fadeIn 0.5s ease-out';
    }
    const tensionValue = document.querySelector('.header-tension-value');
    if (tensionValue) {
      tensionValue.textContent = '0%';
      tensionValue.style.color = '#0066ff';
    }
  }
  function validateStability() {
    if (countryTraits.stability < 0) {
      countryTraits.stability = 0;
    } else if (countryTraits.stability > 100) {
      countryTraits.stability = 100;
    }
  }
  const influenceLevels = {
    LOCAL: {
      ...
      benefits: ["Diplomacy", "Trade"]
    },
    REGIONAL: {
      threshold: 20,
      name: "Regional Power",
      description: "Growing presence",
      benefits: ["Alliances", "Commerce"]
    },
    EMERGING: {
      threshold: 40,
      name: "Emerging Power",
      description: "Rising influence",
      benefits: ["Recognition", "Partners"]
    },
    MAJOR: {
      threshold: 60,
      name: "Major Power",
      description: "Global presence",
      benefits: ["Global ties", "Markets"]
    },
    GREAT: {
      threshold: 80,
      name: "Great Power",
      description: "Dominance",
      benefits: ["Military", "Economy"]
    },
    SUPERPOWER: {
      threshold: 90,
      name: "Superpower",
      description: "Supreme power",
      benefits: ["Leadership", "Control"]
    }
  };
  function getInfluenceLevel(influenceValue) {
    let currentLevel = influenceLevels.LOCAL;
    for (const [key, level] of Object.entries(influenceLevels)) {
      if (influenceValue >= level.threshold) {
        currentLevel = level;
      } else {
        break;
      }
  ...
  function getBenefitDescription(benefit) {
    const descriptions = {
      "Territory control": "Control your core territories",
      "Basic governance": "Basic administration",
      "Cultural growth": "Develop cultural identity",
      "Resources": "Resource management",
      "Authority": "Central power",
      "Taxation": "Generate revenue",
      "Military": "Build armed forces",
      "Expansion": "Control more regions",
      "Army": "Enhanced military power",
      "Trade": "Commercial networks",
      "Global power": "International influence",
      "Tech": "Advanced technology",
      "Economics": "Market dominance",
      "Superpower": "Global superpower",
      "Nuclear": "Nuclear capability",
      "Leadership": "Global politics leader",
      "Ultimate power": "Supreme influence",
      "Military might": "Top military force",
      "Economic rule": "Economic dominance"
    };
    return descriptions[benefit] || "Enhanced capability";
  }
  function reformEmpire() {
    const userCountry = document.getElementById('country-name').value;
    const empireRequirements = {
      "Russia": {
        territories: ["Ukraine", "Georgia"],
        name: "Russian Empire",
        bonus: 50
      },
      "United Kingdom": {
    ...
    const requirements = empireRequirements[userCountry];
    if (!requirements) {
      showAlert("Your nation cannot form a historic empire");
      return;
    }
    const empireDialog = document.createElement('div');
    empireDialog.style.cssText = `
      position: fixed;
      top: 50%;
      left: 50%;
      transform: translate(-50%, -50%);
      ...
      box-shadow: 0 5px 20px rgba(0, 102, 255, 0.2);
    `;
    empireDialog.innerHTML = `
      <h3>Reform ${requirements.name}</h3>
      <p>Required territories:</p>
      <ul>
        ${requirements.territories.map(t => `
          <li>${t} (Relations: ${relationships[userCountry][t] || 0}%)</li>
        `).join('')}
      </ul>
      <p>Bonus: +${requirements.bonus} Influence</p>
      <div style="display: flex; gap: 10px; margin-top: 15px;">
        <button onclick="confirmEmpireReform('${userCountry}')" style="flex: 1">Reform Empire</button>
        <button onclick="this.parentElement.parentElement.remove()" style="flex: 1">Cancel</button>
      </div>
    `;
    document.body.appendChild(empireDialog);
  }
  function confirmEmpireReform(country) {
    const empireRequirements = {
      "Russia": {
        territories: ["Ukraine", "Georgia"],
        name: "Russian Empire",
    ...
    const requirements = empireRequirements[country];
    const canReform = requirements.territories.every(territory => relationships[country][territory] && relationships[country][territory] >= 75);
    if (canReform) {
      countryTraits.influence = Math.min(100, countryTraits.influence + requirements.bonus);
      countryPower.territory += 30;
      countryTraits.stability = Math.min(100, countryTraits.stability + 20);
      worldTension += 20;
      requirements.territories.forEach(territory => {
        relationships[country][territory] = 100;
        relationships[territory][country] = 100;
      });
      addNewsItem(`${requirements.name} Reformed!`, 'critical', `${country} has successfully reformed the historic ${requirements.name}, dramatically increasing their global influence and power.`);
      updateTraits();
      updateRank();
      updateWorldTension();
      saveGameState();
      showAlert(`Successfully reformed the ${requirements.name}!`);
    } else {
      showAlert("Cannot reform empire - improve relations with required territories first");
    }
    document.querySelector('div[style*="position: fixed"]').remove();
  }
  function showInstructions() {
    const backdrop = document.createElement('div');
    backdrop.className = 'modal-backdrop';
    backdrop.onclick = closeInstructionsModal;
    const modal = document.createElement('div');
    modal.className = 'instructions-modal';
    modal.innerHTML = `
      <button class="close-btn" onclick="closeInstructionsModal()">&times;</button>
      <h2>Welcome to Country Builder</h2>
      
      <div class="instruction-section">
        <h3>Getting Started</h3>
        <p>Begin your journey by establishing your nation. Choose a strategic name and select your preferred form of government. Your first critical decision will be selecting your territory on the world map to establish your homeland.</p>
      </div>
      
      <div class="instruction-section">
        <h3>Building Your Power</h3>
        <p>Strengthen your nation through multiple channels:</p>
        <p><strong>Military:</strong> Build a powerful armed forces to defend your interests</p>
        <p><strong>Economy:</strong> Develop a robust economy to fund your ambitions</p>
        <p><strong>Technology:</strong> Advance your nation's capabilities through research</p>
      </div>
      
      <div class="instruction-section">
        <h3>Diplomatic Relations</h3>
        <p>Master the art of international diplomacy:</p>
        <p><strong>Alliances:</strong> Form strategic partnerships with other nations</p>
        <p><strong>Trade:</strong> Establish beneficial economic relationships</p>
        <p><strong>Influence:</strong> Shape global politics through your actions</p>
      </div>
      
      <div class="instruction-section">
        <h3>World Events</h3>
        <p>React to dynamic global situations:</p>
        <p><strong>Crises:</strong> Navigate international tensions and conflicts</p>
        <p><strong>Opportunities:</strong> Seize moments to expand your influence</p>
        <p><strong>Challenges:</strong> Face and overcome threats to your nation</p>
      </div>
  
      <div class="instruction-section">
        <h3>Victory Paths</h3>
        <p>Multiple routes to greatness await:</p>
        <p><strong>Economic Dominance:</strong> Become a global financial powerhouse</p>
        <p><strong>Military Supremacy:</strong> Lead the world's strongest armed forces</p>
        <p><strong>Diplomatic Victory:</strong> Rule through influence and alliances</p>
      </div>
    `;
    document.body.appendChild(backdrop);
    document.body.appendChild(modal);
  ...
  function setTheme(theme) {
    ...
    if (icon) {
      icon.src = theme === 'light' ? 'icons/Sun.svg' : 'icons/Moon.svg';
    }
  }
  ...
  function showRandomNotification() {
    const notifications = ["Don't forget to check your diplomatic relations!", "Your military might need reinforcements", "Consider expanding your territory", "Check the world tension meter", "Form new alliances to increase your power", "Develop your economy through the shop", "Enact new laws to shape your nation"];
